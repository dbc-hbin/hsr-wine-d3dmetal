...
**기존 사용자는 Yaagl을 종료한 뒤 v1.0.4 설치 프로그램으로 다시 설치하고 해당 Wine을 선택하세요.** Wine 아카이브만 교체해서는 런처의 실행 코드가 수정되지 않습니다. 설치 프로그램과 업데이트 helper를 재빌드했으며, 동봉된 Wine 아카이브는 v1.0.2/v1.0.3과 동일합니다. Tahoe 실기기 DX12 검증은 아직 수행하지 않았습니다.
